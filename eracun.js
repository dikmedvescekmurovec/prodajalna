...
streznik.get('/', function(zahteva, odgovor) {
  console.log("ID ob nalaganju strani: " + zahteva.session.idStranke);
...
// Vrni podrobnosti stranke iz seje
var strankaIzSeje = function(idStranke, callback){
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka, polja, datoteke) {
    strankaIzRacuna(polja.seznamRacunov, function(napakaStranka, vrsticeStranka){
      pesmiIzRacuna(polja.seznamRacunov, function(napakaPesmi, vrsticePesmi){
      if (!vrsticePesmi || !vrsticeStranka) {
        odgovor.sendStatus(500);
      } else {
        odgovor.setHeader('content-type', 'text/xml');
        odgovor.render('eslog', {
          vizualiziraj: true,
          postavkeRacuna: vrsticePesmi,
          postavkeStranka: vrsticeStranka
          })
        }
      })
    })
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    strankaIzSeje(zahteva.session.idStranke, function(napakaStranka, vrsticeStranka){
    if (!pesmi || !vrsticeStranka) {
      odgovor.sendStatus(500);
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    console.log("ID po prijavi: " + zahteva.session.idStranke);
    odgovor.redirect('/');
  });
})
...
streznik.post('/odjava', function(zahteva, odgovor) {
    ...
    console.log("ID po odjavi: " + zahteva.session.idStranke);
    odgovor.redirect('/prijava'); 
})
